...
function bindUI(){
  ...
  els.startFishingBtn.onclick=openNextLevel;els.homeReturnGameBtn.onclick=returnToActiveGame;els.howStartBtn.onclick=openNextLevel;els.howToPlayBtn.onclick=()=>showScreen("how");
  els.startLevelBtn.onclick=startCurrentLevel;els.caughtPlayBtn.onclick=openNextLevel;els.caughtReturnGameBtn.onclick=returnToActiveGame;
  els.allModeBtn.onclick=()=>setTrayMode("all");els.edgesModeBtn.onclick=()=>setTrayMode("edges");els.pushModeBtn.onclick=()=>setBoardActionMode("push",true);els.pullModeBtn.onclick=()=>setBoardActionMode("pull",true);els.previewBtn.onclick=togglePreview;
  els.puzzleHomeBtn.onclick=()=>showScreen("home");els.puzzleFishCaughtBtn.onclick=openCaught;
  els.fishAgainBtn.onclick=replayDetailFish;els.nextFishBtn.onclick=openNextLevel;els.resetFishBtn.onclick=resetAllFish;
  ...
  window.addEventListener("pointermove",onPointerMove,{passive:false});window.addEventListener("pointerup",onPointerUp,{passive:false});window.addEventListener("pointercancel",cancelDrag,{passive:false});
...
function renderLevelSelect(){const level=currentLevelNumber();els.levelSelectTitle.textContent=level>maxLevel()?"Collection Complete":`Level ${level}`;els.levelSelectSubtitle.textContent=level>maxLevel()?"You have caught every fish currently available.":"Choose how many pieces you want for this puzzle.";els.difficultyStrip.innerHTML="";difficulties.forEach(d=>{const b=document.createElement("button");b.className="difficulty-choice"+(d.id===currentDifficulty.id?" selected":"");b.innerHTML=`<strong>${d.pieces}</strong><span>${d.label} • ${d.cols} × ${d.rows}</span>`;b.onclick=()=>{currentDifficulty=d;renderLevelSelect()};els.difficultyStrip.appendChild(b)});els.startLevelBtn.disabled=!currentLevelFish();els.startLevelBtn.textContent=currentLevelFish()?`Start Level ${level}`:"All Fish Caught"}
...
function layoutPuzzle(initial=false){
  ...
  const gutter=6,maxW=(rect.width-gutter*2)*.995,maxH=(rect.height-gutter*2)*.995;let boardW=maxW,boardH=boardW*3/4;if(boardH>maxH){boardH=maxH;boardW=boardH*4/3}
...
function renderBoardCutlines(){
  ...
  const {boardW,boardH,cell}=puzzleState.metrics,stroke=Math.max(1.6,cell*.032),col="rgba(27,39,54,.34)";
...
function setBoardActionMode(mode,runAction=false){boardActionMode=mode;updateToolLabels();if(!runAction)return;if(mode==="push")pushVisibleToBoard();else pullLoosePieces()}
...
function pushVisibleToBoard(){if(!puzzleState)return;const list=puzzleState.pieces.filter(p=>p.location==="tray"&&(trayMode==="all"||p.isEdge));if(!list.length){showToast(trayMode==="edges"?"No edge pieces left in the tray.":"Tray is empty.");return}list.forEach(scatterPiece);syncLoosePieces();renderTray();updatePuzzleMeta()}
function pullLoosePieces(){if(!puzzleState)return;const counts=new Map();puzzleState.pieces.filter(p=>p.location==="table"&&!p.locked).forEach(p=>counts.set(p.groupId,(counts.get(p.groupId)||0)+1));const loose=puzzleState.pieces.filter(p=>p.location==="table"&&!p.locked&&counts.get(p.groupId)===1);if(!loose.length){showToast("No loose pieces to pull back.");return}loose.forEach(movePieceToTray);syncLoosePieces();renderTray();updatePuzzleMeta();showToast(`${loose.length} loose piece${loose.length===1?"":"s"} returned`)}
